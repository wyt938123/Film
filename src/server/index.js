...
router.post("/upload/chunk", async (ctx) => {
  try {
    const { fileHash, chunkHash, fileName, chunkIndex, chunkNumber } =
      ctx.request.body;
    ...
    console.log(
      `分片上传成功: ${chunkHash} (${parseInt(chunkIndex) + 1}/${chunkNumber})`,
    );
...
router.get("/upload/check", async (ctx) => {
  try {
    ...
    if (chunkExists) {
      ...
      const uploadedIndexes = uploadedChunks.map((chunk) =>
        chunk.split("-").pop(),
      );
