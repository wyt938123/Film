<template>
    ...
    <div ref="containerRef" class="relative min-h-screen w-full bg-[#ffffff]">
        ...
        <div :style="{
            ...
            columnCount: props.cols ?? 1,
            gap: props.gap ?? 0,
        }" class="bg-yellow absolute left-0 top-0 w-full">
...
const itemDate = computed(() =>
    props.itemDate.map((item, index) => ({ index, ...item })),
);
...
onMounted(() => {
    ...
    phantomHeight.value =
        (itemDate.value.length * estimateHeight.value) / (props.cols ?? 1);
    visibleInfo.startIndex = 0;
    visibleInfo.count =
        Math.ceil(visibleInfo.height / estimateHeight.value) * (props.cols ?? 1) +
        buffer; //导致每次固定截取x个元素，可以优化缓冲区？？
...
function getScrollTop() {
    offSetY.value =
        visibleInfo.startIndex >= 1
            ? itemPositions.value[visibleInfo.startIndex].top
            : 0;
}
...
function handleWindowScroll() {
    ...
    visibleInfo.endIndex = Math.min(
        visibleInfo.startIndex + visibleInfo.count + buffer,
        itemDate.value.length,
    );
    getScrollTop();
