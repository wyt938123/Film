...
import { useWorker } from "@/utils/workerManager";
import type { HashWorkerInput, HashWorkerOutput } from "@/utils/workerTypes";
import {
  uploadChunkApi,
  mergeChunkApi,
  checkFileApi,
} from "@/api/UploadApi/API";
...
const uploadFiles = async function (
  files: FileList,
  uploadFileList: { value: Array<UploadFile> },
  ...
  Array.from(files).forEach(async (element, i) => {
    ...
    const { fileHash, fileChunkList } = await getHashAndChunkWorker(
      file,
      chunkSize,
    );
    ...
    const checkFileOrGetUploadedChunk = async function (
      fullHash: string,
      fileName: string,
    ) {
      //name用来处理秒传
    ...
    // ✅ 传入完整的 hash
    const checkResult = await checkFileOrGetUploadedChunk(
      fullFileHash,
      file.name,
    );
    ...
    // ✅ 更新初始进度
    inTackArrItem.percentage = Math.floor(
      ((inTackArrItem.finishNumber || 0) / inTackArrItem.allChunkList.length) *
        100,
    );
 ...
 * @returns Promise<HashWorkerOutput>
 */
function getHashAndChunkWorker(
  file: File,
  chunkSize = 1 * 1024 * 1024,
): Promise<HashWorkerOutput> {
  // 获取 hash-worker 的执行器
  const { execute } = useWorker("hash-worker");

  // 准备输入数据
  const input: HashWorkerInput = { file, chunkSize };

  // 执行 Worker 任务,支持超时和进度回调
  return execute<HashWorkerInput, HashWorkerOutput>(input, {
    timeout: 60000, // 60秒超时(大文件可能需要更长时间)
    onProgress: (progress, data) => {
      // 可以在这里处理进度,暂时只打印日志
      console.log(`[Hash计算] 进度: ${progress}%`, data);
    },
  });
}

//独立函数。那么uploadfilelist就必须要传了
const uploadSignleFile = async function (
  ...
  // ✅ 断点续传：检查已上传的分片
  try {
    const checkResult = await checkFileApi(
      uploadFile.fileHash,
      uploadFile.fileName ?? "",
    );
    if (checkResult.data?.code === 200) {
      ...
      if (data?.uploaded) {
        ...
        setTimeout(() => {
          const index = uploadFileList.value.findIndex(
            (item) => item.fileHash === uploadFile.fileHash,
          );
          if (index !== -1) {
      ...
      uploadFile.percentage = Math.floor(
        ((uploadFile.finishNumber || 0) / uploadFile.allChunkList.length) * 100,
      );
      console.log(
        `断点续传：发现 ${uploadedChunks.length} 个已上传分片，当前进度: ${uploadFile.percentage}%`,
      );
    }
  } catch (error) {
  ...
  const uploadChunk = async (chunk: ChunkItem): Promise<void> => {
    ...
    try {
      ...
      const uploadItem = uploadFileList.value.find(
        (item) => item.fileHash === uploadFile.fileHash,
      );
      if (uploadItem) {
        uploadItem.finishNumber = (uploadItem.finishNumber || 0) + 1;
        uploadItem.percentage = Math.floor(
          (uploadItem.finishNumber / chunk.chunkNumber) * 100,
        );
