<template>
    <div class="upload-file-page min-h-screen bg-gray-50">
        ...
        <div class="pt-[46px] px-4 py-6">
            <!-- 上传区域 -->
            <upload-area @handleClick="handleClick" @handleUploadFile="handleUploadFile" ref="fileInput" />
            ...
            <div class="mt-4 space-y-3" v-if="uploadFileList.length > 0">
                <div v-for="item in uploadFileList" :key="item.fileHash" class="bg-white rounded-lg p-4 shadow-sm">
                    <upload-progress-item :item="item" @cancelUpload="cancelUpload" />
                </div>
            </div>
        </div>
    </div>
...
import UploadProgressItem from '@/components/Mine/Upload/UoloadProgressItem.vue';
import { ref } from 'vue';
